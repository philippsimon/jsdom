var dom      = exports.dom = require("./jsdom/level3/index").dom,
    ...
    URL      = require('url'),
    CharsetMatch = require("node-icu-charset-detector").CharsetMatch,
...
exports.env = exports.jsdom.env = function() {
  ...
  processHTML = function(err, html) {
    ...
    if( headers ) {
      window.document._headers = headers;
    }
  ...
  // Handle url/file
    ...
    if (url.hostname) {
      ...
      function(err, res, buffer) {
        ...
        // TODO: check if charset could be also get by reading the meat data of the file or use the header
        var charset = new CharsetMatch(buffer).getName();
        try {
          var html = buffer.toString(charset);
        } catch(x) {
          var charsetConverter = new Iconv(charset, config.encoding || "utf8");
          var html = charsetConverter.convert(buffer).toString();
        }
        processHTML(err, html);
      });
    } else {
